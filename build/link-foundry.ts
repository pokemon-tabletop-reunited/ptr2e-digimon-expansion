...
const symlinkPath = path.resolve(dataPath, "modules", "ptr2e-digimon-expansion");
...
if (symlinkStats) {
    ...
    const proceed: boolean = (
        await prompts({
            ...
            name: "value",
            initial: false,
            message: `A "ptr2e-digimon-expansion" ${atPath} already exists in the "modules" subfolder. Replace with new symlink?`,
        })
    ).value;
    if (!proceed) {
